...
contract Casino is usingOraclize {
  ...
  // Each number has an array of players.
  // Associate each number with a bunch of players
  mapping(uint => address[]) numberBetPlayers;

  // The number that each player has bet for
  mapping(address => uint) playerBetsNumber;

  // Modifier to only allow the execution of function when the bets 
  // are completed.
  modifier onEndGame(){
  ...
  /// @notice Constructor that's used to configure the minimum bet per game and
  ///         the max amount of bets
  ...
  function Casino(uint _minimumBet, uint _maxAmountOfBets) public {
    ...
    // Set the proof of oracize in order to make secure random number generators
  ...
  function bet(uint numberToBet) public payable {
    ...
    // Check that the amount paid is bigger or equal to the minimum bet
  ...
  /// @notice Callback function that gets called by oraclize when the random
  ///         number is generated
  /// @param _queryId The query id that was generated to proofVerify
  /// @param _result String that contains the number generated
  /// @param _proof A string with a proof code to verify the authenticity of
